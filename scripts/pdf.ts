...
const parseArgs = (arglist: string[]): [string[], Record<string, string | boolean>] => {
    const [args, kwargs, name] = arglist.reduce<
        [string[], Record<string, string | boolean>, string | undefined]
    >(
        ([args, kwargs, name], arg) => {
            if (arg.startsWith("--")) {
...
const main = async () => {
    ...
    const timestamp =
        typeof kwargs.timestamp === "string" ? new Date(kwargs.timestamp) : new Date();
    ...
    await Promise.all([write(output, pdf), server.stop(), (await browser).close()]);
};
